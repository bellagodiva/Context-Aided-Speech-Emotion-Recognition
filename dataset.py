...
class Multimodal_Datasets(torch.utils.data.Dataset):
    ...
    '''
    def _load_aufeatures(self, uttid):
        #load normalized features in npy file dict, access key with uttid, return value of array features with shape(length, dim 74)
        if uttid in self.norm_aufeatures:
            norm_aufeatures = self.norm_aufeatures[uttid]
            sampled_norm_aufeatures = [norm_aufeatures[i] for i in range(0,len(norm_aufeatures),5)]
            #print(type(sampled_norm_aufeatures))
            #sampled_norm_aufeatures = torch.stack(sampled_norm_aufeatures)
            #sampled_norm_aufeatures = sampled_norm_aufeatures.squeeze()
            #print(sampled_norm_aufeatures.shape)
            return sampled_norm_aufeatures
        else:
            return None

    def _load_vifeatures(self, uttid):
        #access folder with uttid, load image, return array with shape(length, 80, 80)
        transform_img = transforms.Compose([
                                transforms.Resize((80,80)),
                                transforms.Grayscale(),
                                transforms.ToTensor(),
                                transforms.Normalize(mean = [0.5463],
                                                     std= [0.2346])
                                ])
        imgs=[]
        root = self.video_root
        if path.exists(root+'/'+uttid):
            for frame in os.listdir(root+'/'+uttid):
                if frame.endswith('.jpg'):
                    img = Image.open(root+'/'+uttid+'/'+frame)
                    img = transform_img(img)
                    imgs.append(img)
            sampled_imgs = [imgs[i] for i in range(0,len(imgs),2)]
            sampled_imgs=torch.stack(sampled_imgs)
    ...
    def _create_input(
        ...
        for diaid in tqdm(diaids):
            ...
            for uttid in self.utterance_ordered[diaid]:
                vifeatures=self._load_vifeatures(uttid)
                aufeatures=self._load_aufeatures(uttid)
                if vifeatures is not None and aufeatures is not None:
                    ues.append(self._load_utterance_speaker(uttid, speaker_mode))
                    vis.append(vifeatures)
                    aus.append(aufeatures)
                    ids.append(uttid)
            '''
            ues=[]
            uttids=[]
            ...
            for idx, ue in enumerate(ues):
                ...
                #if (ids[idx]) not in emotion_labels:
                    #continue
                label = self.emotion2id[emotion_labels[uttids[idx]]]
                label = [label]
                ...
                inputs.append({"text": text_input, "label":label})
    ...
    def __getitem__(self, index):
        ...
        return {
            'source': {
                ...
                #'audio': torch.Tensor(self._inputs[index]['audio']),
    ...
    def collater(self, samples):
        '''
        audio_source = [s['source']['audio'] for s in samples]
        audio_sizes = torch.LongTensor([len(s) for s in audio_source])
        audio, audio_mask = self.collater_modal(audio_source, audio_sizes)

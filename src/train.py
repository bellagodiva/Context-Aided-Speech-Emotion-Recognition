...
def initiate(hyp_params, train_loader, valid_loader, test_loader):
    ...
    #model = torch.load('/mnt/hard1/bella/erc/pretrained_models/t_30hist_1e-6.pt')
...
def train_model(settings, hyp_params, train_loader, valid_loader, test_loader):
    ...
    #model.load_state_dict(torch.load('/mnt/hard2/bella/erc/pretrained_models/check_30hist_1e-6'))
        ...
        for i_batch, samples in enumerate(train_loader):
            ...
            if hyp_params.use_cuda:
                with torch.cuda.device(0):
                    text, eval_attr = text.cuda(), eval_attr.cuda()
    ...
    for epoch in range(1, hyp_params.num_epochs+1):
        ...
        if val_loss < best_valid:
            print("Saved model at pre_trained_models/mulerc_1.pt!")
            save_model(hyp_params, model, name='new_transcript30hist_1e-6')
            best_valid = val_loss
    
    #save_model(hyp_params, model, name='t_30hist_1e-6')
    model = load_model(hyp_params, model, name='new_transcript30hist_1e-6')

...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x_in, x_in_k = None, x_in_v = None, key_padding_mask=None):
        ...
        #print('emmbed_scale:', self.embed_scale)
        x = self.embed_scale * x_in
        if self.embed_positions is not None:
            #print('x_in,x:', x_in.shape, x.shape)
        ...
        for layer in self.layers:
            if x_in_k is not None and x_in_v is not None:
                x = layer(x, x_k, x_v, encoder_padding_mask=key_padding_mask)
            else:
                x = layer(x, encoder_padding_mask=key_padding_mask)
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, x_k=None, x_v=None, encoder_padding_mask=None):
        """
        Args:
        ...
        if x_k is None and x_v is None:
            x, _ = self.self_attn(query=x, key=x, value=x, attn_mask=mask, key_padding_mask=encoder_padding_mask)
        else:
            x_k = self.maybe_layer_norm(0, x_k, before=True)
            x_v = self.maybe_layer_norm(0, x_v, before=True) 
            x, _ = self.self_attn(query=x, key=x_k, value=x_v, attn_mask=mask, key_padding_mask=encoder_padding_mask)
